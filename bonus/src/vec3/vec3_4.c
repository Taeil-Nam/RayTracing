...
t_vec3	vec3_refract(t_vec3 uv, t_vec3 n, double etai_over_etat)
{
    ...
    r_out_perp = vec3_mul_scalar(vec3_add(uv, vec3_mul_scalar(n, cos_theta)), etai_over_etat);
    r_out_parallel = vec3_mul_scalar(n, -sqrt(fabs(1.0 - vec3_squared(r_out_perp))));
